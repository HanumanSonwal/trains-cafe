...
const ContactInfo = () => {
  // Set up the form using react-hook-form with Zod validation
  const {
    register,
    handleSubmit,
    setValue,
    ...
    resolver: zodResolver(contactInfoSchema), // Use Zod resolver here
  });
    ...
    >
      ...
      <Form className="mt-5" layout="vertical" onFinish={handleSubmit(onSubmit)}>
        ...
        <Form.Item label="Address">
          <TextEditor
            previousValue=""
            updatedValue={(content) => setValue("address", content)} // Update address value in the form
            height={200}
          />
          {errors.address && <span style={{ color: "red" }}>{errors.address.message}</span>}
        </Form.Item>
