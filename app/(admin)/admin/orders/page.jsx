...
const OrdersTable = () => {
  ...
  const buildQueryParams = useCallback((filters, extra = {}) => {
    ...
    try {
      ...
      if (result.success) {
        const mappedData = result.docs.map((order) => ({
          ...
          deliveryDateTime: order?.deliveryDateTime || order?.delivery_date || null,
  ...
  const handleStatusChange = useCallback(async (value, record) => {
    try {
      ...
      if (result.success) {
        ...
        fetchData(currentPage);
      } else message.error(result.message || "Failed to update order status");
    } catch (error) {
  ...
  const handlePaymentStatusChange = useCallback(async (value, record) => {
    try {
      ...
      if (result.success) {
        ...
        fetchData(currentPage);
      } else message.error(result.message || "Failed to update payment status");
    } catch (error) {
  ...
  const handleExport = useCallback((format) => {
    ...
    window.open(`/api/orders/export${query}`, "_blank");
  }, [filters, buildQueryParams]);
  ...
  const columns = useMemo(() => OrderTableColumns({
    ...
    fetchData,
    currentPage,
  }), [handleStatusChange, handlePaymentStatusChange, fetchData, currentPage]);
