...
import React, { useEffect, useState } from 'react';
import { Modal, Button, Input, Select, Row, Col, TimePicker } from 'antd';
import { useForm, Controller } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import dynamic from 'next/dynamic';
...
const vendorSchema = z.object({
  ...
  Delivery_Charges: z.string().nonempty('Please enter the delivery charges'),
  Min_Order_Value: z.string().nonempty('Please enter the minimum order value'),
  Min_Order_Time: z.string().nonempty('Please select the minimum order time'),
 Working_Time_Start: z.any(), // Change to z.any() to allow moment objects
  Working_Time_End: z.any(), // Change to z.any() to allow moment objects
  Weekly_Off: z.string().nonempty('Please select the weekly off day'),
  Food_Type: z.string().nonempty('Please select the food type'),
  Description: z.string().nonempty('Please enter a Description'),
  Address: z.string().nonempty('Please enter the address'),
  ...
  // Custom validation to ensure working times are not empty
  return data.Working_Time_Start && data.Working_Time_End;
}, { message: 'Please select both start and end working times' });
...
const VendorsForm = ({ open, onCancel, onSubmit, initialValues }) => {
  ...
  useEffect(() => {
    if (initialValues) {
      // Convert working times from strings to moment objects if they exist
      const updatedInitialValues = {
        ...initialValues,
        Working_Time_Start: initialValues.Working_Time_Start ? moment(initialValues.Working_Time_Start, 'HH:mm') : null,
        Working_Time_End: initialValues.Working_Time_End ? moment(initialValues.Working_Time_End, 'HH:mm') : null,
      };
  ...
  const handleFormSubmit = async (data) => {
    const submitData = {
      ...data,
      Working_Time_Start: data.Working_Time_Start ? data.Working_Time_Start.format('HH:mm') : '',
      Working_Time_End: data.Working_Time_End ? data.Working_Time_End.format('HH:mm') : '',
      Working_Time: data.Working_Time_Start && data.Working_Time_End
        ? `${data.Working_Time_Start.format('HH:mm')} - ${data.Working_Time_End.format('HH:mm')}`
        : '',
    };
  
    let response;
    if (initialValues) {
      response = await updateData(`/api/vendors/?id=${initialValues._id}`, submitData);
    } else {
      response = await postData('/api/vendors', submitData);
    }
        ...
        <Row gutter={16}>
              ...
              render={({ field: { value, onChange } }) => (
                <div className="mb-4">
                  ...
                  <Input 
                    ...
                    value={value} 
                    onChange={onChange} 
                    placeholder="Enter minimum order time in minutes"
                  />
                  {errors.Min_Order_Time && <p className="text-red-500">{errors.Min_Order_Time.message}</p>}
                </div>
              )}
          ...
          <Col span={12}>
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <div className="mb-4">
                  <label className="block mb-1">Working Time Start</label>
                  <TimePicker 
                    style={{ width: '100%' }}
                    format="HH:mm"
                    value={value}
                  ...
                  />
                  {errors.Working_Time_Start && <p className="text-red-500">{errors.Working_Time_Start.message}</p>}
                </div>
              )}
            />
          </Col>
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Controller
              ...
              render={({ field: { value, onChange } }) => (
                <div className="mb-4">
                  <label className="block mb-1">Working Time End</label>
                  <TimePicker 
                    style={{ width: '100%' }}
                    format="HH:mm"
                    value={value}
                  ...
                  />
                  {errors.Working_Time_End && <p className="text-red-500">{errors.Working_Time_End.message}</p>}
                </div>
              )}
            />
          </Col>
