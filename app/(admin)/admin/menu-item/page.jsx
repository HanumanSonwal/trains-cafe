...
const TablePage = () => {
  ...
  // Fetch data from API
  const fetchMenuItems = async (page, pageSize, search = "") => {
    try {
  ...
  const handleBulkImportSubmit = (formData) => {
    message.success("Bulk import processed successfully");
    setIsBulkImportModalOpen(false);
    fetchMenuItems(page, pageSize); // Refresh after bulk import
  };
  ...
  const handleDeleteMenuItem = async (key) => {
    try {
      await axios.delete(`/api/menu/${key}`);
      setData(data.filter((item) => item.key !== key));
      setFilteredData(filteredData.filter((item) => item.key !== key));
      message.success("Menu item deleted successfully");
    } catch (error) {
  ...
  const handleSearch = (e) => {
    const value = e.target.value;
    setSearchText(value);
    fetchMenuItems(1, pageSize, value); // Fetch filtered data from API
  };
