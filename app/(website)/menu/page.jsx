  ...
  useEffect(() => {
    const getVendorMenus = async () => {
      ...
      try {
        ...
        setCategories([]); // Set empty categories if there's an error
      } finally {
...
const CartComp = ({ cartItems, item }) => {
  ...
  const handleRemoveFromCart = () => {
    if (cartItem.quantity > 1) {
      dispatch(updateItemQuantity({ id: item._id, quantity: cartItem.quantity - 1 }));
    } else {
      dispatch(updateItemQuantity({ id: item._id, quantity: 0 }));
    }
  };
  ...
  return cartItem ? (
    <div className="flex items-center">
      <Button icon={<MinusOutlined />} onClick={handleRemoveFromCart} className="border-green-500 text-green-500" />
      <InputNumber min={0} max={10} value={cartItem.quantity} className="mx-2 w-12 text-center" readOnly />
      <Button icon={<PlusOutlined />} onClick={handleAddToCart} className="border-green-500 text-green-500" />
    </div>
  ) : (
    <Button onClick={handleAddToCart} className="bg-green-500 text-white hover:bg-green-600">
      ADD +
    </Button>
  );
};
