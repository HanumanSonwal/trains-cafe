...
import { ArrowRightOutlined, LoadingOutlined, ShoppingCartOutlined } from "@ant-design/icons";
import Link from "next/link";
import { useDispatch, useSelector } from "react-redux";
    ...
    <div className="w-full max-w-3xl mx-auto p-4">
      ...
      {loading ? (
        <div className="flex justify-center py-10">
          <Spin indicator={<LoadingOutlined style={{ fontSize: 36, color: "#d6872a" }} spin />} />
        </div>
      ) : (
        <div className="bg-white shadow rounded-xl overflow-hidden mb-8">
          ...
          <div className="p-5">
            ...
            <div className="mt-3 flex items-center gap-3">
              <span className="bg-green-600 text-white px-2 py-1 rounded text-xs">4.5 ⭐</span>
              <span className="text-xs text-gray-500">Pure Veg | Min Order ₹99 | 30 mins</span>
            </div>
      ...
      {!loading && categories.length > 0 ? (
        <Collapse accordion expandIconPosition="end" className="bg-white rounded shadow">
          {categories.map((cat) => (
            <Panel
              key={cat._id}
              header={
                <div className="flex items-center gap-4">
                  ...
                  <h3 className="text-lg font-semibold text-[#704D25]">{cat.categoryName}</h3>
                </div>
              }
            >
...
{totalUniqueItems > 0 && (
 ...
 <Button
  type="primary"
  style={{ backgroundColor: "#d6872a", color: "white", display: "flex", justifyContent: "space-between" }}
